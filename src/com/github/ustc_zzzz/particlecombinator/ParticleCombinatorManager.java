...
import com.github.ustc_zzzz.particlecombinator.util.*;
import com.github.ustc_zzzz.particlecombinator.util.repackage.net.objecthunter.exp4j.Expression;
import com.github.ustc_zzzz.particlecombinator.util.repackage.net.objecthunter.exp4j.ExpressionBuilder;
 ...
 */
@NonnullByDefault
public class ParticleCombinatorManager
{
    ...
    public ParticleCombinatorManager(ParticleCombinator plugin)
    {
        this.plugin = plugin;
    ...
    private Map<String, Element> scan(File file)
    {
        ...
        if (file.isDirectory() || file.mkdirs())
        {
            for (File f : Optional.ofNullable(file.listFiles()).orElse(new File[0]))
            {
                ...
                {
                    String elementRealName = fileName.substring(0, fileName.lastIndexOf(".conf"));
                    try
                    {
                        HoconConfigurationLoader loader = HoconConfigurationLoader.builder().setFile(f).build();
                        CommentedConfigurationNode menuRoot = loader.load().getNode(ParticleCombinator.PLUGIN_ID);
                        newElements.put(elementRealName, Objects.requireNonNull(this.deserializeElement(menuRoot)));
                    }
                    catch (Exception e)
                    {
    ...
    public void saveConfig(CommentedConfigurationNode node) throws IOException
    {
        node.setValue(this.menuDirs)
                .setComment(node.getComment().orElse(this.translation
                        .take("particlecombinator.config.scanDir.comment").toPlain()));
    }
    ...
    private Element deserializeElement(ConfigurationNode node, LinkedList<Map<String, Double>> constants) throws InvalidDataException
    {
        ...
        if (node.hasMapChildren())
        {
            ...
            try
            {
                ...
                {
                    String key = entry.getKey().toString(), value = entry.getValue().getString("");
        ...
        return Objects.isNull(reference) ? new Element.Particle().asElement() : (interpolation, random) ->
        {
            ...
            Text errorMessage = this.translation.take("particlecombinator.display.invalidReference", reference);
