 ...
 */
@NonnullByDefault
public class ParticleCombinatorCommands implements Supplier<CommandCallable>
{
    private static final String SUBTITLE = ParticleCombinator.DESC;
    private static final String VERSION = ParticleCombinator.VERSION;
    private static final String GITHUB_URL = ParticleCombinator.GITHUB_URL;
    private static final String WEBSITE_URL = ParticleCombinator.WEBSITE_URL;

    private final ParticleCombinator plugin;
    ...
    private final ParticleCombinatorTranslation translation;

    public ParticleCombinatorCommands(ParticleCombinator plugin)
    {
        this.plugin = plugin;
        ...
        this.reloadCommand = CommandSpec.builder()
                .permission("particlecombinator.reload")
        ...
        Sponge.getCommandManager().register(plugin, this.get(), "particlecombinator", "pc");
    }

    private CommandResult runReloadCommand(CommandSource src, CommandContext args) throws CommandException
    {
        if (src.hasPermission("particlecombinator.reload"))
        {
            ...
            {
                throw new CommandException(this.translation.take("particlecombinator.reload.errorWhenReloading"), e);
            }
    ...
    private CommandResult runVersionCommand(CommandSource src, CommandContext args) throws CommandException
    {
        ...
        src.sendMessage(this.translation.take("particlecombinator.version.description.title", VERSION));
        src.sendMessage(this.translation.take("particlecombinator.version.description.subtitle", SUBTITLE));
        src.sendMessage(Text.of("================================================================"));
        try
        {
            String gitCommitHash = "@git_hash@";
            // RFC 3339
            Date releaseDate = ParticleCombinator.RFC3339.parse("@release_date@");

            src.sendMessage(this.translation
                    .take("particlecombinator.version.description.line1", releaseDate));
            src.sendMessage(this.translation
                    .take("particlecombinator.version.description.line2", gitCommitHash));
            ...
            src.sendMessage(Text.join(this.translation
                    .take("particlecombinator.version.description.line3", ""), urlWebsite));
            src.sendMessage(Text.join(this.translation
                    .take("particlecombinator.version.description.line4", ""), urlGitHub));
    ...
    {
        if (!src.hasPermission("particlecombinator.list"))
        {
            Text error = this.translation.take("particlecombinator.list.noPermission", src.getName());
        ...
        {
            if (src.hasPermission("particlecombinator.send." + name))
            {
                ++size;
                joiner.add(name);
            }
        }
        src.sendMessage(this.translation.take("particlecombinator.list.overview", size));
    ...
    private CommandResult runDisplayCommand(CommandSource src, CommandContext args) throws CommandException
    {
        ...
        if (Objects.isNull(element) || !src.hasPermission("particlecombinator.send." + name))
        {
            Text error = this.translation.take("particlecombinator.display.notExists", name);
            throw new CommandPermissionException(error);
        }
    ...
    public CommandCallable get()
    {
        return CommandSpec.builder()
                ...
                .description(this.translation.take("particlecombinator.description")).build();
    }
}
